...
CORRECT_PASSWORD = "mohammad salam"

def show_main_form():
    global root
    global productnameEntry, productcodeEntry, quantityEntry, priceEntry
    global discountEntry, totalEntry, subtotalEntry, gstEntry, grandtotalEntry
    # === Main Form ===
...
def submit_form():
    ...
    discount = discountEntry.get().strip()
    subtotal = subtotalEntry.get().strip()
...
def password_prompt():
    ...
    pw_window.configure(bg="blue")

    tk.Label(pw_window, text="Enter the password to access form", bg="lightblue").pack(pady=10)
    pw_entry = tk.Entry(pw_window, show="*")
    pw_entry.pack()

    def check_password():
        if pw_entry.get() == CORRECT_PASSWORD:
            pw_window.destroy()
            show_main_form()
        else:
